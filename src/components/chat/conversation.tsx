			...
			<AIConversation className="relative h-full w-full overflow-y-hidden">
				<ConversationContent className="flex w-full flex-col items-center pt-20 pb-40">
					{sortedMessages?.map((message, index) => {
						...
						const combinedText =
							textParts.map((p) => p.text).join("") || textContent;
						...
						return (
							...
							>
								<AIMessage from={message.role}>
									<AIMessageContent className="group-[.is-user]:rounded-3xl group-[.is-assistant]:bg-transparent group-[.is-user]:bg-secondary group-[.is-assistant]:p-0 group-[.is-user]:text-foreground">
										{hasReasoning && (
											<Reasoning
												className="w-full"
										...
										{isAssistant && hasRenderableContent && (
											<Actions className="mt-2 opacity-0 transition-opacity group-hover:opacity-100">
												{(() => {
													const { provider, display, tooltip } =
														getModelDisplay(
