...
function getPlaygroundListWithMeta(): Array<{ id: string; name: string; createdAt: number }> {
...
function getPlaygroundSummary(playgroundId: string): { model: string; messageCount: number } | null {
...
export function AppSidebar({ collapsed, ...props }: React.ComponentProps<typeof Sidebar> & { collapsed?: boolean }) {
  const [playgroundList, setPlaygroundList] = useState<Array<{ id: string; name: string; createdAt: number }>>([]);
  const [summaries, setSummaries] = useState<Record<string, { model: string; messageCount: number }>>({});
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === "chaichat_playground_list" || e.key?.startsWith("chaichat_playground_")) {
        refreshList();
      }
    };
    ...
    try {
      ...
      if (group) {
        const parsed = JSON.parse(decodeURIComponent(group)) as Array<{ id: string; name: string; createdAt: number }>;
  ...
  async function handleConfirmDelete() {
    ...
    try {
      await Promise.all([
        ...
        db.playgroundMessages.where("playgroundId").equals(playgroundToDelete).delete(),
      ]);
    } catch {}
  ...
  return (
    <Sidebar className={`${collapsed ? "left-14" : "left-64"} z-20 h-2/3 pt-16`} variant="floating" {...props}>
      <SidebarHeader>
        <SidebarMenu>
          <SidebarMenuItem>
      ...
      <SidebarContent>
        ...
        <SidebarGroup>
          <div className="px-3 pt-2 pb-1 text-xs font-semibold text-muted-foreground">Playground History</div>
          <SidebarMenu className="gap-1">
            {playgroundList.length === 0 && (
              <SidebarMenuItem>
                <SidebarMenuButton disabled>No playgrounds yet</SidebarMenuButton>
              </SidebarMenuItem>
            )}
            {playgroundList.map(({ id, name, createdAt }) => (
              <SidebarMenuItem key={id} className="group relative">
                <SidebarMenuButton asChild>
                  <Link
                    href={`/playground/${id}`}
                    className="flex flex-col items-start w-full pr-8"
                    onClick={() => {
                      try {
                        if (typeof window !== "undefined") {
                          sessionStorage.setItem("cc_force_open_playground_sidebar", "1");
                  ...
                  >
                    <span className="font-medium text-xs">{name.startsWith('playground-') ? `Playground (${formatDate(createdAt)})` : name}</span>
                    <span className="font-mono text-[10px] text-muted-foreground">{id}</span>
                    <span className="text-xs text-muted-foreground">
                      Model: {summaries[id]?.model || "-"} | Messages: {summaries[id]?.messageCount ?? 0}
                    </span>
                  </Link>
                </SidebarMenuButton>
                ...
                <button
                  type="button"
                  className="absolute right-2 top-1/2 -translate-y-1/2 hidden group-hover:flex items-center justify-center text-muted-foreground hover:text-destructive"
