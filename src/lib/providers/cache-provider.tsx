...
export function CacheProvider({ children }: { children: React.ReactNode }) {
  ...
  const onMessagesChangedCallback = useRef<((chatId: string) => void) | undefined>(undefined);
  ...
  const switchMessageVersionMutation = useMutation(api.chat.switchMessageVersion);
  const markAsOriginalVersionMutation = useMutation(api.chat.markAsOriginalVersion);
  ...
  useEffect(() => {
    const initializeCache = async () => {
      ...
      try {
        // Dexie: Load chats
        const localChats = await db.chats.where("userId").equals(user.id).toArray();
        setChats(localChats);
  ...
  const createChat = useCallback(async (name: string, model: string): Promise<string> => {
    ...
    try {
      ...
      setChats(prev => prev.map(chat => 
        chat._id === optimisticId ? realChat : chat
      ));
  ...
  const deleteChat = useCallback(async (chatId: string): Promise<void> => {
    ...
    try {
      ...
      console.log(`Successfully deleted chat ${chatId} and ${messagesToDelete.length} messages`);
    } catch (error) {
  ...
  const updateChatModel = useCallback(async (chatId: string, model: string): Promise<void> => {
    setChats(prev => prev.map(chat => 
  ...
  const setOnMessagesChanged = useCallback((callback: (chatId: string) => void) => {
    onMessagesChangedCallback.current = callback;
  }, []);

  const markAsOriginalVersion = useCallback(async (messageId: string): Promise<void> => {
    try {
