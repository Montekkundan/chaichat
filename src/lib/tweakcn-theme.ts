...
export function applyTweakcnTheme(themeState: TweakcnThemeState, bypassThrottle: boolean = false) {
  if (typeof window === "undefined") return;
  ...
  const isModeChangeOnly = lastThemeState && 
  ...
  if (!bypassThrottle && !isModeChangeOnly && now - lastAppliedTime < APPLY_THROTTLE_MS) return;
  ...
  if (!isModeChangeOnly) {
    try {
      if (currentStyles) {
        ...
        Object.entries(currentFonts).forEach(([type, fontValue]) => {
          if (fontValue) {
        ...
        Promise.all(fontPromises).then(() => {
          setTimeout(() => {
            ...
            if (fontSansValue) {
              ...
              setTimeout(() => root.style.setProperty('--font-sans', fontSansValue), 10);
            }
            if (fontSerifValue) {
              root.style.setProperty('--font-serif', 'inherit');
              setTimeout(() => root.style.setProperty('--font-serif', fontSerifValue), 10);
            }
            if (fontMonoValue) {
              root.style.setProperty('--font-mono', 'inherit');
              setTimeout(() => root.style.setProperty('--font-mono', fontMonoValue), 10);
            }
          }, 100);
        });
      }
...
export async function fetchTweakcnTheme(url: string): Promise<TweakcnThemeState | null> {
  try {
