...
export class UserSessionManager {
  ...
  setCurrentSession(userId: string, isLoggedIn = false): void {
    ...
    localStorage.setItem(SESSION_TIMESTAMP_KEY, this.currentSession.timestamp.toString());
  }
  ...
  private async syncLocalToConvex(userId: string): Promise<void> {
    try {
      ...
      console.log("Syncing local data to Convex for user:", userId, "chats:", localChats.length);
    } catch (error) {
  ...
  private async clearLocalAndSyncFromConvex(userId: string): Promise<void> {
    try {
      ...
      console.log("Cleared local data and syncing from Convex for user:", userId);
    } catch (error) {
