...
export class ChatTitlesCookieManager {
	...
	static getChatTitles(cookieValue?: string): Map<string, string> {
		...
		try {
			const entries: ChatTitleEntry[] = JSON.parse(decodeURIComponent(cookieValue));
	...
	static setChatTitle(chatId: string, title: string): void {
		...
		try {
			...
			const titleMap = this.getChatTitles(existingCookie);
			
			// Update or add the title
			titleMap.set(chatId, title);
			
			// Convert back to array format with timestamps
			const entries: ChatTitleEntry[] = Array.from(titleMap.entries()).map(([id, name]) => ({
				id,
				name,
				timestamp: Date.now()
			}));
			...
			const expires = new Date(Date.now() + this.MAX_AGE).toUTCString();
			
			document.cookie = `${this.COOKIE_NAME}=${cookieValue}; expires=${expires}; path=/; SameSite=Lax`;
		} catch (error) {
	...
	static getChatTitle(chatId: string, cookieValue?: string): string | null {
		const titleMap = this.getChatTitles(cookieValue);
