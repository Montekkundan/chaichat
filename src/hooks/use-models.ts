...
let memoryCache: Record<Source, { data: LLMGatewayModel[] | null; timestamp: number }> = {
...
export function useLLMModels(options?: {
  ...
  const isControlled = Boolean(options?.controlled) && typeof options?.source === "string";
  const controlledUseAiGateway = options?.source === "aigateway";

  const fetchModels = useCallback(async (overrideUseAiGateway?: boolean) => {
		setIsLoading(true);
		setError(null);
		try {
      const effectiveUseAiGateway = typeof overrideUseAiGateway === "boolean"
        ? overrideUseAiGateway
        : isControlled
          ? controlledUseAiGateway
          : useAiGateway;
      const route = effectiveUseAiGateway ? "/api/models/ai-gateway" : "/api/models";
    ...
    memoryCache = { llm: { data: null, timestamp: 0 }, aigw: { data: null, timestamp: 0 } };
		await fetchModels();
	}, [fetchModels]);
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const source: Source = (isControlled ? controlledUseAiGateway : useAiGateway) ? "aigw" : "llm";
    const memData = memoryCache[source].data;
    const memFresh = memData && now - memoryCache[source].timestamp < CACHE_DURATION_MS;
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("modelsSourceChanged", onToggle as EventListener);
